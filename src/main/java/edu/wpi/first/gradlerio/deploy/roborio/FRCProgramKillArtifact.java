...
public class FRCProgramKillArtifact extends AbstractArtifact {
  ...
  public void deploy(DeployContext ctx) {
    ...
    try (DigestInputStream it =
            new DigestInputStream(
                ToolInstallTask.class.getResourceAsStream("/frcKillRobot.sh"), md);
      ...
      if (result != null && result.toLowerCase().startsWith(local.toLowerCase())) {
        ...
        try (InputStream out =
            new ByteArrayInputStream(dump.getBackingArray(), 0, dump.getBackingLength())) {
          ctx.put(out, "/usr/local/frc/bin/frcKillRobot.sh.tmp");
        }
        ctx.execute(
            "mv /usr/local/frc/bin/frcKillRobot.sh.tmp /usr/local/frc/bin/frcKillRobot.sh && chmod +x /usr/local/frc/bin/frcKillRobot.sh");
        ctx.execute("sync");
      }
    } catch (IOException e) {
