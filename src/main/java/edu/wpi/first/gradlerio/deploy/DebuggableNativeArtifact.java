...
public class DebuggableNativeArtifact extends NativeExecutableArtifact
  ...
  public TargetDebugInfo getTargetDebugInfo() {
    ...
    if (ctx.getController() instanceof IPSessionController) {
      ...
      ToolchainDiscoverer toolchain =
          getTarget()
              .getProject()
              .getExtensions()
              .getByType(ToolchainExtension.class)
              .getToolchainDescriptors()
              .getByName(RoboRioToolchainPlugin.toolchainName)
              .discover();
      ...
      for (HeaderExportingSourceSet sourceSet :
          bin.getInputs().withType(HeaderExportingSourceSet.class)) {
      ...
      return new NativeTargetDebugInfo(
          getName(),
          debugPort,
          session.getHost(),
          getDeployedFile().getAbsolutePath(),
          gdbpath,
          sysroot,
          srcpaths,
          headerpaths,
