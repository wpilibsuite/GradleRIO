  ...
  public FRCNativeArtifact(String name, RoboRIO target) {
    ...
    getBinary()
        .set(
            componentSpec.map(
                x -> {
                  for (NativeExecutableBinarySpec bin :
                      x.getBinaries().withType(NativeExecutableBinarySpec.class)) {
                    if (bin.getTargetPlatform()
                            .getName()
                            .equals(getTarget().getTargetPlatform().get())
    ...
    programStartArtifact =
        target
            .getArtifacts()
            .create("programStart" + name, FRCProgramStartArtifact.class, art -> {});
