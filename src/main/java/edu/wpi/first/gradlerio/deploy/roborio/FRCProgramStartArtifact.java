...
public class FRCProgramStartArtifact extends AbstractArtifact {
  ...
  public void deploy(DeployContext ctx) {
    ...
    String result =
        ctx.execute(
                "/usr/local/natinst/bin/nirtcfg --file=/etc/natinst/share/ni-rt.ini --get section=systemsettings,token=NoApp.enabled,value=unknown")
            .getResult();
    if (result != null && result.trim().equalsIgnoreCase("true")) {
      ctx.getLogger()
          .logError(
              "NoApp is set on the device. Robot program cannot be started. Disable NoApp either with the imaging tool or by holding the User button for 5 seconds");
    } else {
      ctx.execute(
