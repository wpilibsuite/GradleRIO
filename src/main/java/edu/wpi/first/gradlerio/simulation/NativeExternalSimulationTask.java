...
public class NativeExternalSimulationTask extends DefaultTask {
  ...
  public static class SimInfo {
    ...
    public SimInfo(
        String name,
        List<HalSimPair> extensions,
        String launchfile,
        boolean clang,
        Map<String, String> environment,
        List<File> srcpaths,
        List<File> headerpaths,
        List<File> libpaths,
        List<File> libsrcpaths) {
      this.name = name;
  ...
  public void execute() throws IOException {
    ...
    for (NativeBinarySpec binary : binaries) {
      ...
      String name =
          binary.getComponent().getName() + " (in project " + getProject().getName() + ")";
      ...
      for (HeaderExportingSourceSet sourceSet :
          binary.getInputs().withType(HeaderExportingSourceSet.class)) {
      ...
      List<File> libpaths =
          new ArrayList<>(
              Arrays.asList(
                  install.getInstallDirectory().get().getAsFile().listFiles(f -> f.isDirectory())));
      ...
      List<HalSimPair> extensions =
          sim.getHalSimLocations(libpaths, binary.getBuildType().getName().contains("debug"));
