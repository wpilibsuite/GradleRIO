...
public class GradleRIOPlugin implements Plugin<Project> {
    private static boolean _registered_build_finished = false;
            ...
            project.getGradle().buildFinished(result -> {
                ...
                if (result.getFailure() != null) {
                    try {
                        ...
                        System.out.println("Error during build failure checking: " + t.getClass() + ": " + t.getMessage());
                    }
                }
            });
        }
    ...
    private void checkBuildFailed(Project project, BuildResult result) {
        ...
        if (result.getFailure() instanceof MultipleBuildFailures) {
            rootExceptions.addAll(((MultipleBuildFailures)result.getFailure()).getCauses());
        } else {
            rootExceptions.add(result.getFailure());
        }
            ...
            if (t instanceof TaskExecutionException) {
                ...
                if (task instanceof TargetDiscoveryTask) {
                    ...
                    if (reasons.add(("Target" + target.getName()).hashCode())) {
                        ...
                        logger.logError("GradleRIO detected this build failed due to not being able to find \"" + target.getName() + "\"!");
                    ...
                    if (task instanceof AbstractNativeCompileTask) {
                        ...
                        int indexOfPlatform = reasonID.indexOf(StringGroovyMethods.capitalize(typedTask.getTargetPlatform().get().getName()));
