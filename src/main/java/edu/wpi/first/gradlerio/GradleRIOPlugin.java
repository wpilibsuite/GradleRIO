...
public abstract class GradleRIOPlugin implements Plugin<Project> {
  ...
  public abstract static class BuildListener implements FlowAction<BuildListener.Parameters> {
  ...
  private static final String GRADLERIO_DISABLE_CACHE_CLEANUP_PROPERTY =
      "gradlerio.disable.cache.cleanup";
  ...
  private void disableCacheCleanup(Project project) {
    ...
    if (cleanupProp != null && cleanupProp.equals("false")) {
      logger.logErrorHead(
          "Warning! You have the property gradlerio.disable.cache.cleanup set to false");
      logger.logError(
    ...
    try {
      File gradleProperties =
          new File(project.getGradle().getGradleUserHomeDir(), "gradle.properties");
      ...
      logger.logError(
          "Could not disable gradle cache cleanup. Run with --info for more information.");
    ...
    project.allprojects(
        proj -> {
          if (!project.hasProperty("skip-inspector-" + WrapperInspector.NAME)) {
            logger.info(
                "Running " + WrapperInspector.NAME + " inspector on project " + project.getPath());
      ...
      if (t instanceof SingletonTask) {
        ...
        if (visited.add(singletonName)) {
          Logging.getLogger(this.getClass())
              .info("Singleton Task Using: " + t + " for " + singletonName);
        } else {
          Logging.getLogger(this.getClass())
              .info("Singleton Task Disabling: " + t + " for " + singletonName);
          t.setEnabled(false);
        }
      }
    }
      ...
      if (t instanceof TaskExecutionException) {
        ...
        if (task instanceof TargetDiscoveryTask) {
          ...
          if (reasons.add(("Target" + target.getName()).hashCode())) {
            ...
            logger.logError(
                "GradleRIO detected this build failed due to not being able to find \""
                    + target.getName()
                    + "\"!");
          ...
          if (task instanceof AbstractNativeCompileTask) {
            ...
            int indexOfPlatform =
                reasonID.indexOf(
                    StringGroovyMethods.capitalize(typedTask.getTargetPlatform().get().getName()));
            reasonID =
                reasonID.substring(0, indexOfPlatform < 0 ? reasonID.length() : indexOfPlatform);
