...
public class WPINativeExtension {
  ...
  public WPINativeExtension(Project project, WPIExtension wpi, WPIVersionsExtension versions) {
    ...
    te.getCrossCompilers()
        .named(
            nte.getWpi().platforms.roborio,
            c -> {
    ...
    simulationTaskRelease =
        project.getTasks().register("simulateNativeRelease", NativeSimulationTask.class);
    simulationTaskDebug =
        project.getTasks().register("simulateNativeDebug", NativeSimulationTask.class);
    ...
    releaseExternalSimulationTask =
        project
            .getTasks()
            .register(
                "simulateExternalNativeRelease",
                NativeExternalSimulationTask.class,
                t -> {
                  t.getSimulationFile()
                      .set(project.getLayout().getBuildDirectory().file("sim/release_native.json"));
                });

    debugExternalSimulationTask =
        project
            .getTasks()
            .register(
                "simulateExternalNativeDebug",
                NativeExternalSimulationTask.class,
                t -> {
                  t.getSimulationFile()
                      .set(project.getLayout().getBuildDirectory().file("sim/debug_native.json"));
                });

    releaseExternalTestSimulationTask =
        project
            .getTasks()
            .register(
                "testExternalNativeRelease",
                NativeExternalSimulationTask.class,
                t -> {
                  t.getSimulationFile()
                      .set(
                          project.getLayout().getBuildDirectory().file("test/release_native.json"));
                });

    debugExternalTestSimulationTask =
        project
            .getTasks()
            .register(
                "testExternalNativeDebug",
                NativeExternalSimulationTask.class,
                t -> {
                  t.getSimulationFile()
                      .set(project.getLayout().getBuildDirectory().file("test/debug_native.json"));
                });
  }
}
