...
public class SimulationExtension {
  ...
  public SimulationExtension(
    ...
    Callable<Set<File>> debugCallable =
        () -> debugArtifactView.getFiles().getAsFileTree().matching(filterable).getFiles();
    Callable<Set<File>> releaseCallable =
        () -> releaseArtifactView.getFiles().getAsFileTree().matching(filterable).getFiles();
    ...
    releaseDependencySet =
        objects.newInstance(SimulationDependencySet.class, releaseFileCollection);
  }
    ...
    for (HalSimExtension halExt : getHalExtensions()) {
      ...
      for (File base : basePaths) {
        ...
        if (simFile.exists()) {
          extensions.add(
              new HalSimPair(
                  halExt.getName(), simFile.getAbsolutePath(), halExt.getDefaultEnabled().get()));
          break;
        }
      }
    }
