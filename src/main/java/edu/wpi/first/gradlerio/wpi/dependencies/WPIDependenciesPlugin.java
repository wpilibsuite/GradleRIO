...
public class WPIDependenciesPlugin implements Plugin<Project> {
  ...
  public void apply(Project project) {
    ...
    TaskProvider<PreemptiveDownloadTask> lazyPreempt =
        project.getTasks().register("downloadDepsPreemptively", PreemptiveDownloadTask.class);

    lazyPreempt.configure(
        x -> {
          Provider<Set<File>> filesProvider =
              project.provider(
                  () -> {
                    return getFiles();
                  });
  ...
  private Set<File> getFiles() {
    ...
    for (RemoteTarget target :
        project.getExtensions().getByType(DeployExtension.class).getTargets()) {
      for (Artifact artifact : target.getArtifacts()) {
        if (artifact instanceof FRCJREArtifact) {
          ...
          Configuration cfg =
              ((FRCJavaArtifact) artifact).getNativeZipArtifact().getConfiguration().get();
