...
public class WPIJavaDepsExtension {
  ...
  private void createJavaDependencies(
      String groupdId, String artifactId, Provider<String> version) {
    wpilibDeps.add(
        providers.provider(
            () -> {
  ...
  private Provider<String> createJniDependency(
      String groupdId,
      String artifactId,
  ...
  public WPIJavaDepsExtension(WPIVersionsExtension versions, ProviderFactory providers) {
    ...
    createJavaDependencies(
        "edu.wpi.first.cameraserver", "cameraserver-java", versions.getWpilibVersion());
    ...
    createJavaDependencies(
        "edu.wpi.first.thirdparty.frc2024.opencv", "opencv-java", versions.getOpencvVersion());
    createJavaDependencies("org.ejml", "ejml-simple", versions.getEjmlVersion());

    createJavaDependencies(
        "com.fasterxml.jackson.core", "jackson-annotations", versions.getJacksonVersion());
    createJavaDependencies(
        "com.fasterxml.jackson.core", "jackson-core", versions.getJacksonVersion());
    createJavaDependencies(
        "com.fasterxml.jackson.core", "jackson-databind", versions.getJacksonVersion());
  ...
  private List<Provider<String>> getWpilibJniInternal(boolean debug, String platform) {
    return List.of(
        createJniDependency(
            "edu.wpi.first.hal", "hal-cpp", versions.getWpilibVersion(), debug, platform),
        createJniDependency(
            "edu.wpi.first.wpimath", "wpimath-cpp", versions.getWpimathVersion(), debug, platform),
        createJniDependency(
            "edu.wpi.first.ntcore", "ntcore-cpp", versions.getWpilibVersion(), debug, platform),
        createJniDependency(
            "edu.wpi.first.cscore", "cscore-cpp", versions.getWpilibVersion(), debug, platform),
        createJniDependency(
            "edu.wpi.first.thirdparty.frc2024.opencv",
            "opencv-cpp",
            versions.getOpencvVersion(),
            debug,
            platform),
        createJniDependency(
            "edu.wpi.first.wpinet", "wpinet-cpp", versions.getWpilibVersion(), debug, platform),
        createJniDependency(
            "edu.wpi.first.wpiutil", "wpiutil-cpp", versions.getWpilibVersion(), debug, platform),
        createJniDependency(
            "edu.wpi.first.apriltag",
            "apriltag-cpp",
            versions.getWpilibVersion(),
            debug,
            platform));
  }
}
