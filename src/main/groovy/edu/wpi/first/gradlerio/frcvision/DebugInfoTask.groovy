...
class DebugInfoTask extends DefaultTask {
    ...
    void writeDebugInfo() {
        def cfg = []
        FRCVisionPlugin.deployExtension(project).artifacts.all { Artifact art ->
            if (art instanceof FRCVisionJavaArtifact) {
            ...
            } else if (art instanceof FRCVisionNativeArtifact) {
                art.targets.all { String target ->
                    cfg << [
                        artifact: "${art.name} (in project ${project.name})".toString(),
                        target: target,
                        component: (art as FRCVisionNativeArtifact).component,
                        debugfile: "${art.name}_${target}.debugconfig".toString(),
                        language: "cpp"
                    ]
                }
